...
(function($) {
  $.widget('nemac.mapCompare', {
    ...
    _deployApp: function(config, elemId) {
      ...
      if (config['type'] === "fade") {
        $(elemId).find('.fader').append('<label class="slide-label">both</label><div class="slide-group"><span class="slide-label">' + config['leftSelector']['slideLabel'] + '</span><input type="range" min="0" max="1" step="0.5" value="' + config['defaultSlidePosition'] + '" /><span class="slide-label">' + config['rightSelector']['slideLabel'] + '</span></div>');
